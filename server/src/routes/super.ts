...
router.post('/admins', async (req: Request, res: Response) => {
  try {
    ...
    const admin = await prisma.user.create({
      data: { email: email.toLowerCase(), name, passwordHash, passwordPlain: password, role },
    });
...
router.patch('/admins/:id', async (req: Request, res: Response) => {
  try {
    ...
    const admin = await prisma.user.update({ where: { id: req.params.id }, data });
  ...
  try {
    ...
    if (user.role === 'AFFILIATE') {
      ...
      const totalStats = await prisma.order.aggregate({
        ...
        _sum: { orderTotal: true, commissionEarned: true }, _count: true,
      });
      ...
      const pendingPayouts = await prisma.payout.aggregate({ where: { affiliateId: userId, status: 'PENDING' }, _sum: { amount: true } });
      const paidPayouts = await prisma.payout.aggregate({ where: { affiliateId: userId, status: 'PAID' }, _sum: { amount: true } });
      ...
      return res.json({
        user, type: 'affiliate',
        data: {
          codes: codes.map((c) => ({ id: c.id, code: c.code, label: c.label, discountPercent: c.discountPercent, active: c.active, expiresAt: c.expiresAt, timesUsed: c._count.orders })),
          stats: {
          ...
          orders: orders.map((o) => ({ id: o.id, customerFirstName: o.customerFirstName, itemsSummary: o.itemsSummary, orderTotal: o.orderTotal, commissionEarned: o.commissionEarned, discountCode: o.discountCode?.code, codeLabel: o.discountCode?.label, source: o.source, date: o.createdAt })),
        },
      });
    }

    // Admin view
    const [totalAffiliates, activeAffiliates, totalOrders, attributedOrders, totalRevenue, totalCommissions] = await Promise.all([
    ...
    return res.json({
      user, type: 'admin',
      data: { totalAffiliates, activeAffiliates, totalOrders, attributedOrders, totalRevenue: totalRevenue._sum.orderTotal || 0, totalCommissions: totalCommissions._sum.commissionEarned || 0 },
    });
  } catch (error) {
